...
func NewWrapperClient(kubernetesClient kubernetes.Interface, wafClient *waf.Client, lbClient *loadbalancer.LoadBalancerClient, certificatesClient *certificate.CertificatesClient, containerEngineClient *containerengine.ContainerEngineClient, loggingClient ...*logging.Client) *WrapperClient {
	var logClient *logging.Client
	if len(loggingClient) > 0 {
		logClient = loggingClient[0]
	}
	return &WrapperClient{kubernetesClient: kubernetesClient, wafClient: wafClient, lbClient: lbClient, loggingClient: logClient, certificatesClient: certificatesClient, containerEngineClient: containerEngineClient}
}
